...
use wicked_waifus_data::{gacha_pool_data, gacha_view_info_data, text_map_data, GachaPoolData};
use wicked_waifus_protocol::{
    ...
    GachaUsePoolResponse, WeaponItem, WeaponItemAddNotify,
};
...
pub fn on_gacha_request(player: &mut Player, request: GachaRequest, response: &mut GachaResponse) {
    ...
    match gacha_service.pull(player, request.gacha_id, request.gacha_times) {
        Ok(results) => match consume_tides(player, request.gacha_id, request.gacha_times) {
            Ok(_) => {
                let summary = process_gacha_results(&results);
...
fn update_player_inventory(player: &mut Player, summary: RewardSummary) {
    ...
    if !summary.weapons.is_empty() {
        let mut new_weapons: Vec<WeaponItem> = Vec::new();

        for weapon in &summary.weapons {
            match player.inventory.add_weapon(
                weapon.id,
                weapon.func_value,
                weapon.weapon_level,
                weapon.weapon_exp,
                weapon.weapon_breach,
                weapon.weapon_reson_level,
                None,
            ) {
                Ok(incr_id) => {
                    new_weapons.push(WeaponItem {
                        id: weapon.id,
                        incr_id,
                        func_value: weapon.func_value,
                        weapon_level: weapon.weapon_level,
                        weapon_exp: weapon.weapon_exp,
                        weapon_breach: weapon.weapon_breach,
                        weapon_reson_level: weapon.weapon_reson_level,
                        role_id: 0,
                    });
                }
                Err(_) => {
        ...
        player.notify(WeaponItemAddNotify {
            weapon_item_list: new_weapons,
            add_from_role: false,
            reason: 0,
        });
    }
